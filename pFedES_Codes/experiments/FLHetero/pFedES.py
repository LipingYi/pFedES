...
from experiments.FLHetero.Models.CNNs import CNN_1, Generator
...
def train(data_name: str, data_path: str, classes_per_node: int, num_nodes: int, fraction: float,
    ...
    init_net_paras = copy.deepcopy(net.state_dict())
    ...
    with open(str(save_path / f"pFedES_miu_{miu}_N{num_nodes}_C{fraction}_R{steps}_E{epochs}_B{bs}_NonIID_{data_name}_class_{classes_per_node}.csv"), 'w', newline='') as file:
